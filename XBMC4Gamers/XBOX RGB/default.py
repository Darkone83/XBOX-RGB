...
# 16-color preset palette (RRGGBB ints) - Enhanced names only
COLOR_PALETTE = [
    ('Pure White',       0xFFFFFF),
    ('Off/Black',        0x000000),
    ('Xbox Green',       0x00FF00),
    ('Bright Red',       0xFF0000),
    ('Electric Blue',    0x0000FF),
    ('Yellow',           0xFFFF00),
    ('Hot Pink',         0xFF00FF),
    ('Cyan',             0x00FFFF),
    ('Orange',           0xFFA500),
...
# Quick presets - simple addition
QUICK_PRESETS = [
    ('Xbox Classic', {'mode': 0, 'brightness': 180, 'colorA': 0x00FF00}),
    ('Gaming Mode', {'mode': 3, 'brightness': 220, 'speed': 120, 'colorA': 0x00FF00, 'colorB': 0xFF0000}),
    ('Party Mode', {'mode': 4, 'brightness': 255, 'speed': 150}),
    ('Ambient', {'mode': 1, 'brightness': 80, 'speed': 60, 'colorA': 0xFFD8B0}),
    ('Off', {'masterOff': True}),
]
...
def _get_color_name(rgb_value):
    """Get friendly name for color value"""
    for name, value in COLOR_PALETTE:
        if value == rgb_value:
            return name
    return _hex24(rgb_value)
...
# -------------------- Simple menu app - MINIMAL UX IMPROVEMENTS --------------------
class App(object):
    def __init__(self):
        self.net = UdpClient()
        self.dev = None
    ...
    # ---- device discovery / selection (UNCHANGED) ----
    ...
    # ---- menu helpers - ENHANCED ----
    def _title(self):
        dev = '%s @ %s' % (self.dev.get('name','?'), self.dev.get('ip','?')) if self.dev else 'No device'
        ver = self.cfg.get('buildVersion') or self.dev.get('ver') or ''
        status = 'OFF' if self.cfg.get('masterOff') else 'ON'
        if ver:
            return 'XBOX RGB - %s  [v%s] - %s' % (dev, ver, status)
        return 'XBOX RGB - %s - %s' % (dev, status)

    def _mode_name(self, v):
        try: return MODE_NAMES[MODE_VALUES.index(int(v))]
        except Exception: return 'Solid'

    # ---- NEW: quick preset system ----
    def apply_quick_preset(self):
        preset_names = [name for name, _ in QUICK_PRESETS]
        sel = dlg_select('Quick Presets', preset_names)
        if sel >= 0:
            name, cfg = QUICK_PRESETS[sel]
            for key, value in cfg.items():
                self.cfg[key] = value
            _notify('XBOX RGB', 'Applied: %s' % name)
            self.preview()

    # ---- color palette picker (enhanced with names) ----
    def pick_color_from_palette(self, title, current_value):
        """Show 16 preset colors with names and current selection marker."""
        current_idx = -1
        for i, (name, rgb) in enumerate(COLOR_PALETTE):
            if rgb == current_value:
                current_idx = i
                break
        ...
        for i, (name, rgb) in enumerate(COLOR_PALETTE):
            ...
            items.append('%-12s  %s%s' % (name, _hex24(rgb), marker))
    ...
    # ---- edit operations (mostly unchanged, some enhanced) ----
    def set_mode(self):
        # Find current mode for highlighting
        current_mode = int(self.cfg.get('mode', 0))
        current_idx = -1
        try:
            current_idx = MODE_VALUES.index(current_mode)
        except:
            pass
        ...
        for i, name in enumerate(MODE_NAMES):
            marker = ' <--' if i == current_idx else ''
            options.append(name + marker)
        
        sel = dlg_select('Effect Mode', options)
        if sel >= 0:
    ...
    def set_slider(self, key, title, lo, hi):
        ...
        val = dlg_numeric(0, '%s (%d-%d) Current: %d' % (title, lo, hi, cur), str(cur))
        try:
            if val not in (None, ''):
    ...
    def set_palette(self):
        current = int(self.cfg.get('paletteCount', 2))
        options = []
        for i in range(1, 5):
            marker = ' <--' if i == current else ''
            options.append('%d Color%s%s' % (i, 's' if i > 1 else '', marker))
        
        sel = dlg_select('Palette Size', options)
        if sel >= 0:
    ...
    def set_counts(self):
        ...
        for i in range(4):
            cur = int(counts[i])
            val = dlg_numeric(0, '%s (0-50) Current: %d' % (labels[i], cur), str(cur))
            try:
                if val not in (None, ''):
    ...
    # ---- master off / custom playlist (unchanged) ----
    ...
    # ---- Cerbios-inspired categorized menu system ----
    def lighting_effects_menu(self):
        """Lighting Effects - inspired by Cerbios subcategory structure"""
        while True:
            c = self.cfg
            
            entries = [
                'Power Control: %s' % ('OFF' if c.get('masterOff') else 'ON'),
                'Effect Mode: %s' % self._mode_name(c.get('mode',0)),
                '-----',
                'Brightness: %d' % int(c.get('brightness',128)),
                'Animation Speed: %d' % int(c.get('speed',128)),
                'Effect Intensity: %d' % int(c.get('intensity',128)),
                'Effect Width/Gap: %d' % int(c.get('width',5)),
                '-----',
                '< Back to Main Menu'
            ]
            
            sel = dlg_select('Lighting Effects', entries)
            if sel < 0 or sel == len(entries)-1:
                break
            elif sel == 0: self.toggle_master_off()
            elif sel == 1: self.set_mode()
            elif sel == 3: self.set_slider('brightness','Brightness',1,255)
            elif sel == 4: self.set_slider('speed','Animation Speed',0,255)
            elif sel == 5: self.set_slider('intensity','Effect Intensity',0,255)
            elif sel == 6: self.set_slider('width','Effect Width/Gap',1,20)

    def color_configuration_menu(self):
        """Color Configuration - inspired by Cerbios LED color selection"""
        while True:
            c = self.cfg
            
            entries = [
                'Primary Color: %s' % _get_color_name(c.get('colorA',0x00FF00)),
                'Secondary Color: %s' % _get_color_name(c.get('colorB',0x0000FF)),
                'Accent Color C: %s' % _get_color_name(c.get('colorC',0x000000)),
                'Accent Color D: %s' % _get_color_name(c.get('colorD',0x000000)),
                '-----',
                'Active Palette Size: %d colors' % int(c.get('paletteCount',2)),
                '-----',
                '< Back to Main Menu'
            ]
            
            sel = dlg_select('Color Configuration', entries)
            if sel < 0 or sel == len(entries)-1:
                break
            elif sel == 0: self.set_color('colorA','Primary Color')
            elif sel == 1: self.set_color('colorB','Secondary Color')
            elif sel == 2: self.set_color('colorC','Accent Color C')
            elif sel == 3: self.set_color('colorD','Accent Color D')
            elif sel == 5: self.set_palette()

    def channel_hardware_menu(self):
        """Channel & Hardware - inspired by Cerbios hardware settings"""
        while True:
            c = self.cfg
            counts = c.get('count', [0,0,0,0])
            rv = list(c.get('reverse', [False,False,False,False])) + [False,False,False,False]
            
            entries = [
                'Channel 1: %d LEDs, Reverse %s' % (int(counts[0]), 'ON' if rv[0] else 'OFF'),
                'Channel 2: %d LEDs, Reverse %s' % (int(counts[1]), 'ON' if rv[1] else 'OFF'),
                'Channel 3: %d LEDs, Reverse %s' % (int(counts[2]), 'ON' if rv[2] else 'OFF'),
                'Channel 4: %d LEDs, Reverse %s' % (int(counts[3]), 'ON' if rv[3] else 'OFF'),
                '-----',
                'Set All Channel Counts',
                '-----',
                '< Back to Main Menu'
            ]
            
            sel = dlg_select('Channel & Hardware', entries)
            if sel < 0 or sel == len(entries)-1:
                break
            elif sel == 0: self.channel_detail_menu(0)
            elif sel == 1: self.channel_detail_menu(1)
            elif sel == 2: self.channel_detail_menu(2)
            elif sel == 3: self.channel_detail_menu(3)
            elif sel == 5: self.set_counts()

    def channel_detail_menu(self, channel):
        """Individual channel configuration"""
        while True:
            c = self.cfg
            counts = list(c.get('count', [0,0,0,0])) + [0,0,0,0]
            rv = list(c.get('reverse', [False,False,False,False])) + [False,False,False,False]
            
            entries = [
                'LED Count: %d' % int(counts[channel]),
                'Reverse Direction: %s' % ('ON' if rv[channel] else 'OFF'),
                '-----',
                '< Back'
            ]
            
            sel = dlg_select('Channel %d Configuration' % (channel+1), entries)
            if sel < 0 or sel == len(entries)-1:
                break
            elif sel == 0:
                cur = str(int(counts[channel]))
                val = dlg_numeric(0, 'Channel %d LED Count (0-50) Current: %d' % (channel+1, int(counts[channel])), cur)
                try:
                    if val not in (None, ''):
                        counts[channel] = max(0, min(50, int(val)))
                        self.cfg['count'] = counts[:4]
                        self.preview()
                except: pass
            elif sel == 1:
                self.toggle_reverse(channel)

    def system_integration_menu(self):
        """System Integration - inspired by Cerbios system settings"""
        while True:
            c = self.cfg
            ...
            entries = [
                ...
                '-----',
                'CPU Temperature Display: %s' % ('ON' if c.get('enableCpu') else 'OFF'),
                'Fan Control Integration: %s' % ('ON' if c.get('enableFan') else 'OFF'),
                '-----',
                'Custom Playlist Loop: %s' % ('ON' if c.get('customLoop') else 'OFF'),
                'Edit Custom Playlist (JSON)',
                '-----',
                '< Back to Main Menu'
            ]
            
            sel = dlg_select('System Integration', entries)
            if sel < 0 or sel == len(entries)-1:
                break
            elif sel == 0: self.toggle('resumeOnBoot','Resume on Boot')
            elif sel == 2: self.toggle('enableCpu','CPU Temperature Display')
            elif sel == 3: self.toggle('enableFan','Fan Control Integration')
            elif sel == 5: self.toggle_custom_loop()
            elif sel == 6: self.edit_custom_seq()

    def device_management_menu(self):
        """Device Management - inspired by Cerbios device info display"""
        while True:
            status = self._title()
            
            entries = [
                'Current Device: %s' % (self.dev.get('name','None') if self.dev else 'None'),
                'Device IP: %s' % (self.dev.get('ip','None') if self.dev else 'None'),
                'Device Version: %s' % (self.dev.get('ver','Unknown') if self.dev else 'Unknown'),
                '-----',
                'Rescan Network',
                'Manual IP Entry',
                'Set Device Password',
            ...
            sel = dlg_select('Device Management', entries)
            if sel < 0 or sel == len(entries)-1:
                break
            elif sel == 4: self.rescan()
            elif sel == 5: self.set_manual_ip()
            elif sel == 6: self.set_psk()
            elif sel == 7: self.test_connection()

    # ---- main menu - Cerbios-inspired categorized structure ----
    def run(self):
        if not self.pick_device():
            return
        
        while True:
            # Main category menu inspired by Cerbios structure
            entries = [
                'Quick Presets',
                'Lighting Effects',
                'Color Configuration',
            ...
            if sel < 0 or sel == len(entries) - 1:  # Exit
                break
            elif sel == 0: self.apply_quick_preset()
            elif sel == 1: self.lighting_effects_menu()
            elif sel == 2: self.color_configuration_menu()
            elif sel == 3: self.channel_hardware_menu()
            elif sel == 4: self.system_integration_menu()
            elif sel == 5: self.device_management_menu()
            elif sel == 7: # Preview Changes
                self.preview()
                _notify('XBOX RGB', 'Preview applied')
            elif sel == 8: self.save()
            elif sel == 9: self.reset()
